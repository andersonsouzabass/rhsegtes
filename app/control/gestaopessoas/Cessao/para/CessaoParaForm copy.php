 ...
 */
class CessaoParaForm extends TWindow
{
    protected $form; // form
    ...
    public function __construct()
    {
        ...
        // creates the form
        $this->form = new BootstrapFormBuilder('form_Cessao_Para');
        $this->form->setFormTitle('Cessão De Outros Orgãos');
        ...
        $detail_vinculo_id = new TDBCombo('detail_vinculo_id', 'gratifica', 'vinculo', 'id', 'nome', null, $criteria_ativo, TRUE);
        $detail_simbolo_id = new TDBCombo('detail_simbolo_id', 'gratifica', 'simbolo', 'id', 'nome', null, $criteria_ativo, TRUE);
        $detail_cargo_id = new TDBCombo('detail_cargo_id', 'gratifica', 'Cargo', 'id', 'nome', null, $criteria_ativo, TRUE);
        $detail_funcao_id = new TDBCombo('detail_funcao_id', 'gratifica', 'Funcao', 'id', 'nome', null, $criteria_ativo, TRUE);

        $detail_especialidade_id = new TCombo('detail_especialidade_id'); //ok 

        $detail_regime_trabalho_id = new TDBCombo('detail_regime_trabalho_id', 'gratifica', 'Regime', 'id', 'nome', null, $criteria_ativo, TRUE);
        $detail_unidade_id = new TDBCombo('detail_unidade_id', 'gratifica', 'ViewUnidadeUsuario', 'id', 'nome', null, TCriteria::create( ['system_user_id' => $id_logado] )); //ok
        ...
        $detail_situacao->setValue('A DISPOSIÇÃO DE OUTRO ORGÃO');
        ...
        $detail_matricula->setEditable(FALSE);
        $detail_conselho_desc->setEditable(FALSE);
        $detail_conselho_num->setEditable(FALSE);
        $detail_ativo->setEditable(FALSE);
        $detail_dt_admissao->setEditable(FALSE);
        $detail_instrumento_legal->setEditable(FALSE);
        $detail_simbolo_id->setEditable(FALSE);
        $detail_cargo_id->setEditable(FALSE);
        $detail_funcao_id->setEditable(FALSE);
        $detail_especialidade_id->setEditable(FALSE);
        $detail_unidade_id->setEditable(FALSE);
        $detail_regime_trabalho_id->setEditable(FALSE);
        $detail_situacao->setEditable(FALSE);
        //Fim dos campos bloqueados
        ...
        //[ new TLabel('Unidade'), $detail_unidade_id ],
        ...
        $row = $this->form->addFields(  [ new TLabel('Orgão de Destino'), $detail_orgao_cedente_destino_id ],
                                        [ new TLabel('Tipo de Cessão'), $detail_tipo_cessao ],
        ...
        $add = TButton::create('add', [$this, 'onDetailAdd'], 'Registrar Cessão', 'fa:plus-circle green');
        ...
        $this->detail_list->addColumn( new TDataGridColumn('cargo', 'Cargo', 'left', 100) );
        $this->detail_list->addColumn( new TDataGridColumn('especialidade', 'Especialidade', 'left', 100) );

        $this->detail_list->addColumn( new TDataGridColumn('tipo_cessao', 'Tipo Cessão', 'left', 100) );
        $this->detail_list->addColumn( new TDataGridColumn('orgao_cedente_destino_id', 'Orgão Destino', 'left', 100) );
        $this->detail_list->addColumn( new TDataGridColumn('forma_cessao', 'Forma de Cessão', 'left') )->setVisibility(false);

        $this->detail_list->addColumn( new TDataGridColumn('funcao', 'Função', 'left') )->setVisibility(false);
        $this->detail_list->addColumn( new TDataGridColumn('unidade', 'Unidade', 'left', 100) )->setVisibility(false);
        ...
        $this->form->addHeaderActionLink( _t('Close'),  new TAction(['CessaoParaList', 'onReload']), 'fa:times red' );
        
        $btn = $this->form->addAction(_t('Save'), new TAction([$this, 'SalvarCessao'], ['static'=>'1']), 'fa:save');
        $btn->class = 'btn btn-sm btn-primary';
       
        $this->form->addActionLink('Cancelar',  new TAction(['CessaoParaList', 'onReload']),  'fa:times red' );
    ...
    public function onDetailAdd( $param )
    {
        try
        {
            ...
            TForm::sendData( 'form_Cessao_Para', $data, false, false );
        }
        catch (Exception $e)
        {
    ...
    public static function onDetailEdit( $param )
    {
        ...
        $criteria = TCriteria::create( ['cargo_id' => $param['cargo_id'] ] );
            ...
            TDBCombo::reloadFromModel('form_Cessao_Para', 'detail_especialidade_id', 'gratifica', 'ViewEspecialidadeCargo', 'id', 'nome', null, $criteria, TRUE);
        ...
        try
        {
            ...
            if( isset($param['ServidorVinculo_list_vinculo_id'] ))
            {
                foreach( $param['ServidorVinculo_list_vinculo_id'] as $key => $item_id )
                {
                    ...
                    $ctd_vinc->vinculo_id  = $param['ServidorVinculo_list_vinculo_id'][$key];
                    $ctd_vinc->simbolo_id  = $param['ServidorVinculo_list_simbolo_id'][$key];
                    $ctd_vinc->cargo_id  = $param['ServidorVinculo_list_cargo_id'][$key];
                    $ctd_vinc->funcao_id  = $param['ServidorVinculo_list_funcao_id'][$key];
                    $ctd_vinc->especialidade_id  = $param['ServidorVinculo_list_especialidade_id'][$key];
                    $ctd_vinc->unidade_id  = $param['ServidorVinculo_list_unidade_id'][$key];
                    $ctd_vinc->regime_trabalho_id  = $param['ServidorVinculo_list_regime_trabalho_id'][$key];
                    $ctd_vinc->matricula  = $param['ServidorVinculo_list_matricula'][$key];
                    $ctd_vinc->conselho_desc  = $param['ServidorVinculo_list_conselho_desc'][$key];
                    $ctd_vinc->conselho_num  = $param['ServidorVinculo_list_conselho_num'][$key];
                    $ctd_vinc->ativo  = $param['ServidorVinculo_list_ativo'][$key];
                    $ctd_vinc->dt_admissao  = $param['ServidorVinculo_list_dt_admissao'][$key];
                    $ctd_vinc->anotacao  = $param['ServidorVinculo_list_anotacao'][$key];
                    $ctd_vinc->instrumento_legal  = $param['ServidorVinculo_list_instrumento_legal'][$key];
                    $ctd_vinc->servidor_id = $master->id;
                    $ctd_vinc->situacao = 'A DISPOSIÇÃO DE OUTRO ORGÃO';

                    $ctd_vinc->tipo_cessao  = $param['ServidorVinculo_list_tipo_cessao'][$key];
                    $ctd_vinc->orgao_cedente_destino_id  = $param['ServidorVinculo_list_orgao_cedente_destino_id'][$key];
                    $ctd_vinc->forma_cessao  = $param['ServidorVinculo_list_forma_cessao'][$key];

                    //new TMessage('info', $param['ServidorVinculo_list_id'][$key]);

                    $EspecFolha = ServidorVinculo::find($param['ServidorVinculo_list_id'][$key]);
                    if ($EspecFolha instanceof ServidorVinculo)
                    {
                        $ctd_vinc->especialidadefolha_id = $EspecFolha->especialidadefolha_id;                                
                    }  
            ...
            TForm::sendData('form_Cessao_Para', (object) ['id' => $master->id]);
            
            new TMessage('info', 'Cessão Cadastrado com Sucesso');
        }
        catch (Exception $e) // in case of exception
        {
            new TMessage('error', $e->getMessage());
    ...
    public static function onListaEspecialidade($param)
    {
        try
        {
            ...
            {
                TDBCombo::clearField('form_Cessao_Para', 'detail_especialidade_id');
            }
