...
class FeriasForm extends TWindow
{
    ...
    public function __construct()
    {
        ...
        // creates the form
        $this->form = new BootstrapFormBuilder('form_ferias');
        $this->form->setFormTitle('Apontamento de Férias');
        ...
        //$detail_tipo_afastamento_id = new TDBUniqueSearch('detail_tipo_afastamento_id', 'gratifica', 'TipoAfastamento', 'id', 'nome');
        ...
        // detail fields
        $this->form->addContent( ['<h4>Período de Férias e Anotação</h4><hr>'] );
        ...
        $add = TButton::create('add', [$this, 'onDetailAdd'], 'Adicionar Férias', 'fa:plus-circle green');
        ...
        $this->detail_list->setId('Ferias_list');
        ...
        $this->form->addHeaderActionLink( 'Fechar',  new TAction(['FeriasList', 'onReload']), 'fa:times red' );
        // create the form actions
        $btn = $this->form->addAction( _t('Save'), new TAction([$this, 'onSave'], ['static'=>'1']), 'fa:save');
        $btn->class = 'btn btn-sm btn-primary';

        $this->form->addActionLink('Cancelar', new TAction(array('FeriasList','onReload')),  'fa:times red' );
    ...
    public function onDetailAdd( $param )
    {
        try
        {
            ...
            TDataGrid::replaceRowById('Ferias_list', $uniqid, $row);
            ...
            TForm::sendData( 'form_ferias', $data, false, false );
        }
        catch (Exception $e)
        {
    ...
    public static function onDetailDelete( $param )
    {
        ...
        // remove row
        TDataGrid::removeRowById('Ferias_list', $param['uniqid']);
    }
    ...
    public function onEdit($param)
    {
        try
        {
            ...
            if (isset($param['key']))
            {
                ...
                $items  = Ferias::where('servidor_vinculo_id', '=', $key)->load();
    ...
    public function onSave($param)
    {
        try
        {
            ...
            Ferias::where('servidor_vinculo_id', '=', $master->id)->delete();
            
            if( isset($param['Ferias_list_id'] ))
            {
                foreach( $param['Ferias_list_id'] as $key => $item_id )
                {
                    $detail = new Ferias;
                    $detail->observacao  = $param['Ferias_list_observacao'][$key];
                    $detail->dt_inicio  = $param['Ferias_list_dt_inicio'][$key];
                    $detail->dt_fim  = $param['Ferias_list_dt_fim'][$key];
            ...
            TForm::sendData('form_ferias', (object) ['id' => $master->id]);
