...
class SystemAdministrationDashboard extends TPage
{
    ...
    function __construct()
    {
        ...
        try
        {
            ...
            $indicator2->enableSection('main', ['title' => 'Perfis',   'icon' => 'users',      'background' => 'blue',   'value' => SystemGroup::count()]);
            $indicator3->enableSection('main', ['title' => 'Unidades de Saúde',    'icon' => 'university', 'background' => 'purple', 'value' => Pessoa::count()]);
            ...
            $stats2 = ViewUnidadeUsuario::groupBy('id')->countBy('system_user_id', 'count');
            
            if ($stats2)
            {
                foreach ($stats2 as $row)
                {
                    $data2[] = [ Pessoa::find($row->id)->nome, (int) $row->count];
                }
            }
            // replace the main section variables
