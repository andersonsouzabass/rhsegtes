 ...
 * 
 */
class AndamentoProcessosList extends TPage
{
    ...
    public function __construct()
    {
        ...
        $this->setDefaultOrder('dt_entrada', 'asc');         // defines the default order
        //$this->setOrderCommand('nome', '(SELECT nome FROM servidor)');
        
        // creates the form
        $this->form = new BootstrapFormBuilder('form_processos_list');
        $this->form->setFormTitle('Processos e Documentos - Atendimento de Processos');
        ...
        $column_id = new TDataGridColumn('id', 'Processo', 'center',  '10%');
        ...
        $column_status = new TDataGridColumn( 'status', 'Situação', 'center');
        ...
        $action1 = new TDataGridAction(['AdamentoProcessosForm', 'onEdit'], ['id'=>'{id}']);
